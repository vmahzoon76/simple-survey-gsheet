...
def highlightable_text(text, key):
    """A custom Streamlit component that displays text and allows highlighting.

    Returns the inner HTML of the component, including any <mark> tags
    that are added by the user.
    """
    # Convert the Markdown text to HTML
    text_html = markdown.markdown(text)

    # The CSS style for the container and the highlighting
    style_css = """
    <style>
        #container {
    ...
    # The JavaScript to handle the highlighting
    highlight_js = f"""
    <script>
    const container = document.getElementById("container");

    // We'll send the state back to Streamlit on mouseup
    container.addEventListener("mouseup", () => {{
        ...
        if (selectedText) {{
            ...
            // This is the key step: it surrounds the selected text with the <mark> tag
            try {{
                range.surroundContents(mark);
            }} catch (e) {{
                // This can happen if the selection spans across different HTML elements.
                // For this simple case, we can ignore it, but a more robust solution
                // might handle this by applying styles differently.
                console.warn("Could not wrap the selection.", e);
                return;
            }}
            ...
            // Send the updated HTML back to Streamlit
            // The Streamlit component framework will listen for this and update the return value
    ...
    # The final HTML body of the component
    component_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        <title>Highlightable Text</title>
        {style_css}
    </head>
    <body>
        <div id="container">{text_html}</div>
    ...
    # Render the component and get its value
    component_value = _html(component_html, height=520, scrolling=False)
    
    # Return the component's current value, or the original HTML if it's not yet set
    return component_value if component_value else text_html
...
resp_headers = [
    ...
    "q_aki", "highlighted_summary_html", "q_rationale", "q_confidence", "q_reasoning"
]
...
with left:
    st.markdown("**Discharge Summary** (highlight relevant text)")
    # Replace st.write(summary) with our new component.
    # It displays the summary AND captures the highlighted version.
    highlighted_summary = highlightable_text(summary, key=f"highlighter_{case_id}")
    ...
    if submitted1:
        try:
            ...
            row = {
                ...
                "step": 1,
                "q_aki": q_aki,
                "highlighted_summary_html": highlighted_summary,
